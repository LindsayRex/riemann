...
# ############################################################################
# ORIGINAL EXPERIMENT2STATS INTERFACE
# ====================================
# 
# Maintaining backward compatibility with orchestrator
# ############################################################################

class Experiment2Stats:
    """Original statistical analysis class for per-configuration processing."""
    
    def __init__(self, verbose=True):
        """Initialize statistical analysis engine."""
        self.verbose = verbose
        if verbose:
            print("Experiment 2 Statistical Analysis initialized")
    
    def analyze_stability_pattern(self, delta_values, delta_E_values, zero_label=""):
        """
        Analyze stability pattern for zero perturbation data.
        
        Args:
            delta_values: Array of perturbation values
            delta_E_values: Array of energy changes
            zero_label: Label for output (e.g., "Zero 1", "Zero 2")
            
        Returns:
            dict: Stability analysis results
        """
        if self.verbose:
            print(f"Analyzing stability pattern for {zero_label}...")
        
        # Convert to numpy arrays for analysis
        deltas = np.array([float(d) for d in delta_values])
        energies = np.array([float(e) for e in delta_E_values])
        
        # Fit quadratic model: ΔE = C₁δ + C₂δ²
        design_matrix = np.column_stack([deltas, deltas**2])
        
        if scipy_available:
            # Use scipy for robust fitting
            def quadratic_model(delta, c1, c2):
                return c1 * delta + c2 * delta**2
            
            try:
                popt, pcov = curve_fit(quadratic_model, deltas, energies)
                C1, C2 = popt
                
                # Calculate R² and other statistics
                y_pred = quadratic_model(deltas, C1, C2)
                ss_res = np.sum((energies - y_pred)**2)
                ss_tot = np.sum((energies - np.mean(energies))**2)
                r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
                
                # Calculate RMSE
                rmse = np.sqrt(np.mean((energies - y_pred)**2))
                
                # Simple p-value estimate for quadratic term
                if len(pcov) > 1:
                    std_err_c2 = np.sqrt(pcov[1, 1])
                    t_stat = abs(C2 / std_err_c2) if std_err_c2 > 0 else 0
                    # Rough p-value approximation
                    p_value = 2.0 * (1.0 - min(0.999, t_stat / 3.0))
                else:
                    p_value = 1.0
                
            except Exception as e:
                if self.verbose:
                    print(f"  Warning: scipy fitting failed ({e}), using manual method")
                # Fallback to manual fitting
                C1, C2, r_squared, p_value, rmse = self._manual_quadratic_fit(deltas, energies)
        else:
            # Manual least squares fitting
            C1, C2, r_squared, p_value, rmse = self._manual_quadratic_fit(deltas, energies)
        
        # Stability criterion: C₂ > 0 (upward curvature)
        is_stable = C2 > 0
        ...
        results = {
            ...
            'rmse': float(rmse),
            'is_stable': is_stable,
            'stability_measure': float(C2),  # Positive = stable
            'sample_size': len(deltas)
        }
        
        if self.verbose:
            print(f"  C₁ = {C1:.3e}, C₂ = {C2:.3e}")
            print(f"  R² = {r_squared:.6f}, RMSE = {rmse:.3e}")
            print(f"  Stability: {'STABLE' if is_stable else 'UNSTABLE'} (C₂ {'>' if is_stable else '≤'} 0)")
        
        return results
    
    def _manual_quadratic_fit(self, deltas, energies):
        """Manual quadratic fitting using normal equations."""
        try:
            # Design matrix for quadratic fit
            A = np.column_stack([deltas, deltas**2])
            ...
            # Simple significance test for C2
            residuals = energies - y_pred
            n = len(deltas)
            if n > 2:
                mse = np.sum(residuals**2) / (n - 2)
                var_c2 = mse * np.linalg.inv(ATA)[1, 1]
                t_stat = abs(C2 / np.sqrt(var_c2)) if var_c2 > 0 else 0
                p_value = 2.0 * (1.0 - min(0.999, t_stat / 3.0))
            else:
                p_value = 1.0
            ...
            C1, C2 = 0.0, 0.0
            r_squared, p_value, rmse = 0.0, 1.0, float('inf')
        
        return C1, C2, r_squared, p_value, rmse

    def analyze_complete_results(self, math_results):
        """
        Run complete statistical analysis on math results.
        
        Args:
            math_results: Results from Experiment2Math
            
        Returns:
            dict: Complete statistical analysis
        """
        if self.verbose:
            print("Running complete two-zero statistical analysis...")
        
        individual = math_results['individual']
        joint = math_results['joint']
        
        # Individual zero analyses
        stability_zero1 = self.analyze_stability_pattern(
            individual['delta_values'], 
            individual['delta_E1_values'],
            "Zero 1"
        )
        
        stability_zero2 = self.analyze_stability_pattern(
            individual['delta_values'],
            individual['delta_E2_values'], 
            "Zero 2"
        )
        
        # Joint perturbation analysis
        stability_joint = self.analyze_stability_pattern(
            joint['delta_values'],
            joint['delta_E12_values'],
            "Joint (Both Zeros)"
        )
        
        # Overall stability assessment
        individual_stable = stability_zero1['is_stable'] and stability_zero2['is_stable']
        joint_stable = stability_joint['is_stable']
        overall_stable = individual_stable and joint_stable
        
        if self.verbose:
            print(f"Overall stability assessment: {'STABLE' if overall_stable else 'UNSTABLE'}")
            print(f"  Individual zeros: {'STABLE' if individual_stable else 'UNSTABLE'}")
            print(f"  Joint perturbation: {'STABLE' if joint_stable else 'UNSTABLE'}")
        
        return {
            'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
            'stability_zero1': stability_zero1,
            'stability_zero2': stability_zero2,
            'stability_joint': stability_joint,
            'overall_stable': overall_stable,
            'individual_stable': individual_stable,
            'joint_stable': joint_stable
        }

# Factory function for backward compatibility
def create_experiment2_stats(verbose=True):
    """Create Experiment2Stats instance."""
    return Experiment2Stats(verbose=verbose)
